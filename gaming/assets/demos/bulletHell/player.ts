...
export class Player extends cObject {
    ...
    @property(cc.Prefab)
    skill:cc.Prefab = null;

    guns: Array<Gun> = [];
    velocity: cc.Vec3 = new cc.Vec3();
    ...
    onSkill(){
        ...
        for(let i = 0;i < 3;i++){
            ...
            this.node.convertToWorldSpaceAR(cc.Vec3.ZERO,tempPos); //局部转世界
            parent.convertToNodeSpaceAR(tempPos,tempPos); //世界转局部
            skill.setPosition(tempPos);//计算发射的起点
            ...
            skill.velocity.x = x*speed;
            skill.velocity.y = y*speed;
            skill.velocity.z = 0*speed;
