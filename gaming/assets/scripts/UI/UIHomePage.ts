...
export default class UIHomePage extends UIPage {
  ...
  protected onLoad() {
    ...
    this.updateNetworkAndWallet();
  }

  updateNetworkAndWallet() {
    ...
    if (isNetworkSelected && isWalletAndAddressSelected) {
      let networkIcon = cc.find("BtnSelectNetwork/icon", this._page);
      let networkName = cc.find("BtnSelectNetwork/name", this._page);
      let walletIcon = cc.find("BtnWallet/icon", this._page);
      let walletName = cc.find("BtnWallet/name", this._page);
      networkName.getComponent(cc.Label).string = network;
      networkIcon.getComponent(cc.Sprite).spriteFrame = cocosz.resMgr.getRes(
        networkMain + "_logo",
        cc.SpriteFrame
      );
      walletName.getComponent(cc.Label).string = address;
      walletIcon.getComponent(cc.Sprite).spriteFrame = cocosz.resMgr.getRes(
        wallet + "_logo",
        cc.SpriteFrame
      );
    }
