...
export abstract class cShape {
    ...
    updateAABB(scale:cc.Vec3 , world: cc.Mat3, isIdentity: boolean = true) {
        ...
        if (!isIdentity) {
            ...
            const m = world.m;
            const uX = m[0] * sx, uY = m[1] * sx, uZ = m[2] * sx;
            const vX = m[3] * sy, vY = m[4] * sy, vZ = m[5] * sy;
            const wX = m[6] * sz, wY = m[7] * sz, wZ = m[8] * sz;
...
export class cPolygon extends cShape {
    constructor(center: cc.Vec3, points:Array<cc.Vec2>) {
